...
import { ChevronLeft, Plus, Search, Dumbbell, X, Save, AlertCircle, Info, Loader2, ArrowRight } from 'lucide-react';
...
export default function CreatePlanPage() {
  ...
  const fetchExercises = async () => {
    if (!token) return;
    try {
      const res = await fetch(`${API_BASE_URL}/api/workout/fetch-public-exercise-move`, {
  ...
  const handleCreateExercise = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/workout/create-exercise`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        if (newExAccessibility === 'public') {
          setPublicExercises(prev => [newEx, ...prev]);
        }
  ...
  const handleSavePlan = async () => {
    ...
    setSavingPlan(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/workout-plan/create`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-4xl mx-auto pb-24">
      ...
      <header className="flex items-center gap-4">
        <button 
          onClick={() => router.back()}
          className="bg-zinc-800 hover:bg-zinc-700 p-2.5 rounded-full text-zinc-300 transition-colors"
        >
          <ChevronLeft size={24} />
        </button>
        <div>
          <h1 className="text-3xl font-bold text-white mb-1">Custom Plan</h1>
          <p className="text-zinc-400 text-sm">Design your perfect workout routine manually.</p>
        </div>
      </header>

      {/* Plan Name */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
        <label className="block text-zinc-400 text-sm font-medium mb-2 uppercase tracking-wide">
          Plan Name
        </label>
        <input 
          type="text"
          ...
          className="w-full bg-zinc-950 border border-zinc-800 focus:border-pink-500/50 rounded-2xl py-4 px-5 text-white text-lg focus:outline-none focus:ring-2 focus:ring-pink-500/20 transition-all"
        />
      </div>

      {/* Days Selector */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
        <h2 className="text-lg font-bold text-white mb-4">Select Day to Edit</h2>
        <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent">
          {DAYS.map((day, idx) => (
            <button
              key={day}
              onClick={() => setSelectedDay(idx)}
              className={`whitespace-nowrap px-6 py-3 rounded-2xl font-medium transition-all flex flex-col items-center gap-1 ${selectedDay === idx ? 'bg-pink-600 text-white shadow-lg shadow-pink-600/20 scale-105 transform origin-bottom' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200'}`}
            >
        ...
        <div className="mt-8">
          ...
          <div className="bg-zinc-950 border border-zinc-800/50 rounded-2xl p-4 min-h-[120px] transition-all">
            {(plannedExercises[selectedDay] && plannedExercises[selectedDay].length > 0) ? (
              <div className="space-y-3">
                {plannedExercises[selectedDay].map((ex, exIdx) => (
                  <div key={`${ex.ExMoveID}-${exIdx}`} className="flex items-center justify-between bg-zinc-900 border border-zinc-800 p-3 rounded-xl animate-in slide-in-from-right-4 duration-300">
                    <div className="flex items-center gap-3">
                      <div className="bg-pink-500/10 p-2 rounded-lg text-pink-500">
                        <Dumbbell size={16} />
                      </div>
                      <div>
                        <p className="text-white text-sm font-medium">{ex.Description}</p>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 text-white px-4 py-2 rounded-xl text-sm font-medium transition-colors"
          >
            <Plus size={16} />
            Create Your Own
          </button>
        ...
        <div className="relative mb-6">
          ...
          <input 
            ...
            className="w-full bg-zinc-950 border border-zinc-800 rounded-xl py-3 pl-11 pr-4 text-zinc-200 focus:outline-none focus:ring-1 focus:ring-pink-500/50"
          />
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent">
            {filteredExercises.map(ex => (
              <div 
                ...
                className="bg-zinc-950 border border-zinc-800 hover:border-pink-500/30 p-4 rounded-2xl cursor-pointer transition-colors group flex justify-between items-center"
              >
                <div>
                  <p className="text-white font-medium group-hover:text-pink-100 transition-colors">{ex.Description}</p>
                  <p className="text-xs text-zinc-500 mt-1">{ex.RecordType === 'time' ? 'Time' : 'Weight/Reps'}</p>
                </div>
                <div className="w-8 h-8 rounded-full bg-zinc-900 flex items-center justify-center text-zinc-400 group-hover:bg-pink-600 group-hover:text-white transition-colors">
                  <Plus size={16} />
                </div>
              </div>
            ))}
      ...
      {/* Bottom Save Bar */}
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-zinc-950/80 backdrop-blur-md border-t border-zinc-800/50 z-40">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="text-sm text-zinc-400 hide-on-mobile">
            {Object.values(plannedExercises).reduce((a, b) => a + b.length, 0)} total exercises
          </div>
          <button 
            onClick={handleSavePlan}
            disabled={savingPlan}
            className="w-full md:w-auto bg-pink-600 hover:bg-pink-500 disabled:opacity-50 disabled:cursor-not-allowed text-white px-8 py-3.5 rounded-full font-bold transition-colors shadow-lg shadow-pink-600/20 flex items-center justify-center gap-2"
          >
            {savingPlan ? <Loader2 size={20} className="animate-spin" /> : <Save size={20} />}
            Confirm Creating Plan
          </button>
        </div>
      </div>
      ...
      {isViewModalOpen && selectedExercise && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-sm overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200">
            <div className="p-6 overflow-y-auto">
              <h3 className="text-xl font-bold text-white mb-4">{selectedExercise.Description}</h3>
              {selectedExercise.Steps && (
                <div className="mb-4">
                  <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-2">Steps</h4>
                  <p className="text-sm text-zinc-300 leading-relaxed">{selectedExercise.Steps}</p>
                </div>
              )}
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 overflow-y-auto space-y-4">
              <div>
                <label className="block text-zinc-400 text-xs font-medium mb-1">Description *</label>
                <input 
                  type="text" 
                  value={newExDesc} onChange={e => setNewExDesc(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-pink-500"
                  placeholder="e.g., Barbell Bench Press"
                />
              </div>
              <div>
                <label className="block text-zinc-400 text-xs font-medium mb-1">Steps *</label>
                <textarea 
                  value={newExSteps} onChange={e => setNewExSteps(e.target.value)}
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:border-pink-500 min-h-[80px]"
                  placeholder="1. Lie on bench... 2. Unrack bar..."
                />
              </div>
              <div>
                <label className="block text-zinc-400 text-xs font-medium mb-1">Caution (Optional)</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-zinc-400 text-xs font-medium mb-1">Video URL (Optional)</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between bg-zinc-950 p-2 rounded-xl border border-zinc-800">
                  <span className="text-zinc-300 text-sm pl-2">Record Type</span>
                  <div className="flex bg-zinc-900 rounded-lg p-1">
                    <button 
                      onClick={() => setNewExRecordType('weight')}
                      className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${newExRecordType === 'weight' ? 'bg-pink-600 text-white' : 'text-zinc-400'}`}
                    >Weight/Reps</button>
                    <button 
                      onClick={() => setNewExRecordType('time')}
                      className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${newExRecordType === 'time' ? 'bg-pink-600 text-white' : 'text-zinc-400'}`}
                    >Time</button>
                  </div>
                </div>

                <div className="flex items-center justify-between bg-zinc-950 p-2 rounded-xl border border-zinc-800">
                  <span className="text-zinc-300 text-sm pl-2">Accessibility</span>
                  <div className="flex bg-zinc-900 rounded-lg p-1">
                    <button 
                      onClick={() => setNewExAccessibility('public')}
                      className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${newExAccessibility === 'public' ? 'bg-pink-600 text-white' : 'text-zinc-400'}`}
                    >Public</button>
                    <button 
                      onClick={() => setNewExAccessibility('private')}
                      className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${newExAccessibility === 'private' ? 'bg-pink-600 text-white' : 'text-zinc-400'}`}
                    >Private</button>
                  </div>
                </div>

                <div className="flex items-center justify-between bg-zinc-950 p-2 rounded-xl border border-zinc-800">
                  <span className="text-zinc-300 text-sm pl-2">Progress Goal</span>
                  <div className="flex bg-zinc-900 rounded-lg p-1">
                    <button 
                      onClick={() => setNewExProgressType('increase')}
                      className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${newExProgressType === 'increase' ? 'bg-pink-600 text-white' : 'text-zinc-400'}`}
                    >Increase</button>
                    <button 
                      onClick={() => setNewExProgressType('decrease')}
                      className={`px-3 py-1 text-xs rounded-md font-medium transition-colors ${newExProgressType === 'decrease' ? 'bg-pink-600 text-white' : 'text-zinc-400'}`}
                    >Decrease</button>
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 border-t border-zinc-800 bg-zinc-900/50 flex gap-3">
              ...
              <button 
                ...
                className="flex-1 bg-pink-600 hover:bg-pink-500 disabled:opacity-50 text-white font-medium py-3 rounded-xl transition-colors text-sm flex justify-center items-center gap-2"
              >
